...
(function() {
    ...
    function showMasterSection(section) {
        $('#sectionPembeli, #sectionIkan, #sectionBongkaran, #sectionMaintenance').hide();
        if (section === 'pembeli') {
            $('#sectionPembeli').show();
            $('#sidebarPembeli').addClass('active');
            $('#sidebarIkan,#sidebarBongkaran,#sidebarMaintenance').removeClass('active');
        } else if (section === 'ikan') {
            $('#sectionIkan').show();
            $('#sidebarIkan').addClass('active');
            $('#sidebarPembeli,#sidebarBongkaran,#sidebarMaintenance').removeClass('active');
            $('#searchIkan').val('');
            displayIkanList('');
        } else if (section === 'bongkaran') {
            $('#sectionBongkaran').show();
            $('#sidebarBongkaran').addClass('active');
            $('#sidebarPembeli,#sidebarIkan,#sidebarMaintenance').removeClass('active');
        } else if (section === 'maintenance') {
            $('#sectionMaintenance').show();
            $('#sidebarMaintenance').addClass('active');
            $('#sidebarPembeli,#sidebarIkan,#sidebarBongkaran').removeClass('active');
        }
    }

    // ==================== MIGRASI KODE TRANSAKSI (DATA LAMA) ====================
    function tampilkanStatusMigrasiKodeTransaksi(html, tipe) {
        var $box = $('#migrasiKodeTransaksiStatus');
        $box.removeClass('alert-info alert-success alert-warning alert-danger')
            .addClass('alert-' + (tipe || 'info'))
            .html(html)
            .show();
    }

    async function cekKodeTransaksiKosong() {
        var $btn = $('#btnCekKodeTransaksi');
        var labelAsli = $btn.html();
        $btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i> Mengecek...');
        tampilkanStatusMigrasiKodeTransaksi('🔍 Mengecek data di server...', 'info');
        try {
            var hasil = await postToServer({ action: 'cekKodeTransaksiKosong' });
            if (hasil.kosong > 0) {
                tampilkanStatusMigrasiKodeTransaksi(
                    '📊 Total ' + hasil.totalBaris + ' baris data. Ditemukan <strong>' + hasil.kosong +
                    ' baris</strong> yang belum punya Kode Transaksi dan perlu dimigrasi.',
                    'warning'
                );
            } else {
                tampilkanStatusMigrasiKodeTransaksi(
                    '✅ Semua ' + hasil.totalBaris + ' baris data sudah punya Kode Transaksi. Tidak ada yang perlu dimigrasi.',
                    'success'
                );
            }
        } catch (err) {
            tampilkanStatusMigrasiKodeTransaksi('⚠️ Gagal mengecek data: ' + err.message, 'danger');
        } finally {
            $btn.prop('disabled', false).html(labelAsli);
        }
    }

    async function jalankanMigrasiKodeTransaksi() {
        var konfirmasi = confirm(
            'Migrasi ini akan mengisi Kode Transaksi pada baris-baris lama yang masih kosong ' +
            '(baris yang sudah punya Kode Transaksi tidak akan diubah).\n\n' +
            'Disarankan data sudah dibackup terlebih dahulu. Proses ini idealnya hanya dijalankan SEKALI ' +
            'dan bisa memakan waktu beberapa saat untuk data yang banyak.\n\n' +
            'Lanjutkan menjalankan migrasi?'
        );
        if (!konfirmasi) return;

        var $btn = $('#btnJalankanMigrasiKodeTransaksi');
        var labelAsli = $btn.html();
        $btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i> Memproses...');
        tampilkanStatusMigrasiKodeTransaksi('⏳ Migrasi sedang berjalan di server, mohon tunggu (jangan tutup halaman ini)...', 'info');
        try {
            var hasil = await postToServer({ action: 'migrasiKodeTransaksiLama' });
            tampilkanStatusMigrasiKodeTransaksi(
                '✅ Migrasi selesai!<br>📊 Total baris: ' + hasil.totalBaris +
                '<br>🆕 Baris baru diisi Kode Transaksi: ' + hasil.diisi +
                '<br>↩️ Baris yang sudah punya kode sebelumnya: ' + hasil.sudahAda,
                'success'
            );
            // Muat ulang data supaya Rekap Data ikut terupdate
            await loadAllData(isFullHistoryLoaded);
        } catch (err) {
            tampilkanStatusMigrasiKodeTransaksi('⚠️ Migrasi gagal: ' + err.message, 'danger');
        } finally {
            $btn.prop('disabled', false).html(labelAsli);
        }
    }
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#sidebarMaintenance', function(e) { e.preventDefault(); showMasterSection('maintenance'); });
        $('#tabMaster').on('shown.bs.tab', function() { showMasterSection('pembeli'); });

        // Alat Bantu: Migrasi Kode Transaksi
        $(document).on('click', '#btnCekKodeTransaksi', cekKodeTransaksiKosong);
        $(document).on('click', '#btnJalankanMigrasiKodeTransaksi', jalankanMigrasiKodeTransaksi);
